...
class C3D(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        ...
        self.conv1 = nn.Conv3d(3, 64, kernel_size=(3, 3, 3), padding=(1, 1, 1)) # 定义卷积层conv1
        self.pool1 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2)) # 定义池化层pool1

        self.conv2 = nn.Conv3d(64, 128, kernel_size=(3, 3, 3), padding=(1, 1, 1))  # 定义卷积层conv2
        self.pool2 = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2))  # 定义池化层pool2

        self.conv3a = nn.Conv3d(128, 256, kernel_size=(3, 3, 3), padding=(1, 1, 1))  # 定义卷积层conv3a
        self.conv3b = nn.Conv3d(256, 256, kernel_size=(3, 3, 3), padding=(1, 1, 1))  # 定义卷积层conv3b
        self.pool3 = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2))  # 定义池化层pool3

        self.conv4a = nn.Conv3d(256, 512, kernel_size=(3, 3, 3), padding=(1, 1, 1))  # 定义卷积层conv4a
        self.conv4b = nn.Conv3d(512, 512, kernel_size=(3, 3, 3), padding=(1, 1, 1))  # 定义卷积层conv4b
        self.pool4 = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2))  # 定义池化层pool4

        self.conv5a = nn.Conv3d(512, 512, kernel_size=(3, 3, 3), padding=(1, 1, 1))  # 定义卷积层conv5a
        self.conv5b = nn.Conv3d(512, 512, kernel_size=(3, 3, 3), padding=(1, 1, 1))  # 定义卷积层conv5b
        self.pool5 = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2), padding=(0, 1, 1))  # 定义池化层pool5
    ...
    def forward(self, x):
        ...
        x = x.contiguous().view(-1, 8192)  # 经过pool5池化以后特征图的大小为(512, 1, 4, 4),利用view函数将其转化为(1,8192)
    ...
    def __load__pretrained_weights(self):
        ...
        p_dict = torch.load('ucf101-caffe.pth')  # 加载预训练权重，预训练权重是一个字典格式，里面包含定义的网络层和对应的权重值
        s_dict = self.state_dict()  # 加载自己定义的网络层的模型名和权重值
